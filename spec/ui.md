...
| Section | Contents |
|---|---|
| **Navbar** | "Edu" wordmark (display font, accent dot), **Tasks \| Grades \| College tab toggle** (center, Fin's Invest/Spending pattern — each tab swaps the whole page body; a separate Tests tab existed for a day on 2026-08-26 and was folded back into the planner), plug icon with connected-count badge |
| **College panel** (2026-08-26 — the cowork registry + degree plan) | **Degree hero**: program + graduation target, big mono done-%, credits meter (emerald banked · accent in course · track ahead). **Week**: the real class timetable — one column per weekday, blocks in class colors (left color edge, code + room + times), today column tinted, accent now-line. **Classes**: one card per registry class — dot, name, ⚑ anchor badge (unlocks), optativa/⚠ flags, `code · turma · cr · pending`, professor, evaluation criteria, schedule lines, workspace deliveries count (`pdf ✓`), platform link-out. **Road to graduation**: the 4 forward semesters as chip columns (status colors from the curriculum). **Curriculum**: P01–P09 rows of code chips — emerald dispensada, accent em curso, zinc a cursar, amber ring ⚠ dispensa at risk (tooltips carry name/planned/notes). **Requirements**: meters for obrigatórias (computed), optativas, humanas, livre, ACE — done + in-course segments. |
