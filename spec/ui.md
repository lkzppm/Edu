...
| Section | Contents |
|---|---|
| **Navbar** | "Edu" wordmark (display font, accent dot), **Tasks \| Grades \| College tab toggle** (center, Fin's Invest/Spending pattern — each tab swaps the whole page body; a separate Tests tab existed for a day on 2026-08-26 and was folded back into the planner), **Claude button** (filled sunburst icon, status dot: zinc unconnected · emerald ok · accent pulse syncing · red error) beside the plug — the Cowork workspace lives HERE in a small popover (classes/deliveries counts, synced-ago, re-scan, disconnect, or the connect action), never in the connectors slide-over (pending/courses semantics don't fit it; the panel filters `cowork` out and the plug badge doesn't count it), then plug icon with connected-count badge |
| **College panel** (2026-08-26; mini-tabs same day — Lucas: one section per view, all visual) | Four **mini-tabs** (segmented pill, keyed → eases in): **fluxogram** — the curriculum drawn as a flowchart: one column per period P01–P09 (h-scroll), header `done/total · cr`, each course a **node card** (code + credits, name, planned/note line) filled by status — emerald dispensada, accent em curso, zinc a cursar, amber ring ⚠ dispensa at risk, ⚑ anchor; **plan** — degree hero (program, graduation target, big mono done-%, credits meter emerald·accent·track) + the 4 forward semesters as node-card columns ("now" on the current one) + requirement meters (obrigatórias computed, optativas, humanas, livre, ACE; done + in-course segments) + contingency note; **week** — the real timetable: one column per weekday, blocks in class colors (left edge, code + room + times), today tinted, accent now-line; **classes info** — one card per registry class: dot, name, ⚑ anchor badge, optativa/⚠ flags, `code · turma · cr · pending`, professor, evaluation criteria, schedule, workspace deliveries (`pdf ✓`), platform link-out. |
